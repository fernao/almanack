...
<!-- artigos -->
<div class="condicional_submit" id="artigos">
  <h4>{translate key="author.submissions.article_head"}</h4>
  <ul>
    <li>{translate key="author.submissions.article_title"}</li>
    <li>{translate key="author.submissions.article_keywords"}</li>
    <li>{translate key="author.submissions.article_maxpages"}</li>
</ul>
</div>

<!-- forum -->
<div class="condicional_submit" id="forum">
  <h4>{translate key="author.submissions.forum_head"}</h4>
  <ul>
    <li>{translate key="author.submissions.forum_main"}</li>
    <li>{translate key="author.submissions.forum_title"}</li>
    <li>{translate key="author.submissions.forum_keywords"}</li>
  </ul>
</div>

<!-- resenhas -->
<div class="condicional_submit" id="resenhas">
  <h4>{translate key="author.submissions.review_head"}</h4>
  <ul>
    <li>{translate key="author.submissions.review_maxpages"}</li>
    <li>{translate key="author.submissions.review_maxyears"}</li>
  </ul>
</div>
	<div class="separator"></div>
{/if}
...
$(document).ready(function(){
  ...
  $('#sectionId').change(
    function(id) {
      arrSection = new Array();
      arrSection[0] = false;
      arrSection[25] = 'forum';
      arrSection[26] = 'artigos';
      arrSection[28] = 'resenhas';
      arrSection[27] = false;
      arrSection[29] = false;
      arrSection[30] = false;
      
      for (var item in arrSection) {
        if (arrSection[item] != false) {     
	    itemHide = "#" + arrSection[item];
	    $(itemHide).hide();
        }
      }
      itemShow = "#" + arrSection[this.value];
      $(itemShow).show();
    }
  );
});
