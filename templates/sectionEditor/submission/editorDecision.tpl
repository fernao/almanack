	...
	<td width="80%" class="value">
		<form method="post" action="{url op="recordDecision"}">
			...
			<select name="decision" size="1" class="selectMenu">
			  {foreach from=$editorDecisionOptions item=optionValue key=optionKey}
			  <option key="{$optionValue}" {if $optionKey == $lastDecision}selected{/if}>{translate key="$optionValue"}</option>
			  {/foreach}
			  {* comentei por hora {html_options_translate options=$editorDecisionOptions selected=$lastDecision} *}
			</select>
