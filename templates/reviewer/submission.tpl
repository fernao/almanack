			...
			<tr valign="top">
				...
				{else}
					...
					<select name="recommendation" {if not $confirmedStatus or $declined or $submission->getCancelled() or (!$reviewFormResponseExists and !$reviewAssignment->getMostRecentPeerReviewComment() and !$uploadedFileExists)}disabled="disabled"{/if} class="selectMenu">
					  {foreach from=$reviewerRecommendationOptions item=optionValue key=optionKey}
					  <option key="{$optionValue}">{translate key="$optionValue"}</option>
					  {/foreach}
{* comentei				{html_options_translate options=$reviewerRecommendationOptions selected=''} *}
					</select>&nbsp;&nbsp;&nbsp;&nbsp;
