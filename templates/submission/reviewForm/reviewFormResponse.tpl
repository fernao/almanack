...
<form id="saveReviewFormResponse" name="saveReviewFormResponse" method="post" action="{url op="saveReviewFormResponse" path=$reviewId|to_array:$reviewForm->getId()}">
...
<!-- almanack - enviar para o conselho -->
{if $canEdit}
{php}

$to = "almanack_braziliense@yahoogrupos.com.br";
$subject = "[Almanack] - parecer";
$this->assign('to', $to);
...
{/php}
				...
				<p>Encaminhar parecer para o conselho: 
{url|assign:"reviewUrl" op="notifyReviewer" reviewId=$reviewId articleId=$articleId}

<input type="hidden" name="to" value="{$to}" />
<input type="hidden" name="subject" value="{$subject}" />
<input type="hidden" id="parecer" name="parecer" value="" />
...
$(document).ready(function(){
  $('#mail').click(function(){
    ...
    var parecerValue = '#reviewFormResponses-2-' + $("input[name='reviewFormResponses[2]']:checked").val();
    
    parecer = $(parecerValue).next('label').text();;
    $('#parecer').attr('value', parecer);
    
    $('#saveReviewFormResponse').attr('action', '/index.php/almanack/user/email');
    $('#saveReviewFormResponse').submit();
  });
});
{/literal}
</script>

<img style="cursor: pointer" id="mail" src="/lib/pkp/templates/images/icons/mail.gif" alt="Correio" height="14" width="16"></a>
                                </p>
{/if}
<!-- fim - almanack -->
