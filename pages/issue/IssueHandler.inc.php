...
class IssueHandler extends Handler {
	...
	function current($args = null) {
		...
		if ($issue != null) {
			...
			$periodicos =& $periodicosDAO->_getPeriodicosFromIssue($issue->getIssueId(), true);
                        $templateMgr->assign('periodicos', $periodicos);
                        // ** almanack fim **//      
			...
			$issueCrumbTitle = Localex::translate('current.noCurrentIssue');
			$issueHeadingTitle = Localex::translate('current.noCurrentIssue');
		}
	...
	function setupIssueTemplate(&$issue, $showToc = false) {
		...
		if (isset($issue) && ($issue->getPublished() || Validation::isEditor($journalId) || Validation::isLayoutEditor($journalId) || Validation::isProofreader($journalId)) && $issue->getJournalId() == $journalId) {
			...
			$issueCrumbTitle = Localex::translate('archive.issueUnavailable');
			$issueHeadingTitle = Localex::translate('archive.issueUnavailable');
		}
	...
	function archive() {
		...
		$templateMgr =& TemplateManager::getManager();
                $templateMgr->assign('title', 'archive.archives');
		$templateMgr->assign('coverPagePath', $coverPagePath);
		$templateMgr->assign('locale', Localex::getLocale());
	...
	function setupTemplate() {
		...
		Localex::requireComponents(array(LOCALE_COMPONENT_PKP_READER, LOCALE_COMPONENT_OJS_EDITOR));
	}
}
