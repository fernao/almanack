	...
	function saveSettings($args, $request) {
		...
		if ($request->getUserVar('uploadSiteStyleSheet')) {
			if (!$settingsForm->uploadSiteStyleSheet()) {
				$settingsForm->addError('siteStyleSheet', Localex::translate('admin.settings.siteStyleSheetInvalid'));
			}
			...
			if (!$settingsForm->uploadPageHeaderTitleImage($settingsForm->getFormLocale())) {
				$settingsForm->addError('pageHeaderTitleImage', Localex::translate('admin.settings.homeHeaderImageInvalid'));
			}
