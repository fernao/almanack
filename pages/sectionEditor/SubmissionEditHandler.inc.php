...
class SubmissionEditHandler extends SectionEditorHandler {
	...
	function submission($args) {
		...
		Localex::requireComponents(array(LOCALE_COMPONENT_PKP_READER, LOCALE_COMPONENT_OJS_AUTHOR));
	...
	function submissionReview($args) {
		...
		Localex::requireComponents(array(LOCALE_COMPONENT_OJS_MANAGER));
	...
	function selectReviewer($args) {
		...
		if (isset($args[1]) && $args[1] != null) {
			...
			$templateMgr->assign('alphaList', explode(' ', Localex::translate('common.alphaList')));
	...
	function enrollSearch($args) {
		$articleId = isset($args[0]) ? (int) $args[0] : 0;
		$this->validate($articleId, SECTION_EDITOR_ACCESS_REVIEW);
		Localex::requireComponents(array(LOCALE_COMPONENT_PKP_MANAGER)); // manager.people.enrollment, manager.people.enroll
		$submission =& $this->submission;
...
print 1;
	...
	function viewMetadata($args, $request) {
		...
		$this->validate($articleId);
		Localex::requireComponents(array(LOCALE_COMPONENT_OJS_AUTHOR));
		$submission =& $this->submission;
	...
	function removeArticleCoverPage($args, &$request) {
		...
		$formLocale = $args[1];
		if (!Localex::isLocaleValid($formLocale)) {
	...
	function uploadSuppFile($fileName = null, $request) {
		...
		$suppFileForm->setData('title', array($submission->getLocale() => Localex::translate('common.untitled')));
