...
class EditorHandler extends SectionEditorHandler {
	...
	function index($args) {
		...
		$templateMgr->assign('sectionOptions', array(0 => Localex::Translate('editor.allSections')) + $sections);
	...
	function submissions($args) {
		...
		$filterEditorOptions = array(
			FILTER_EDITOR_ALL => Localex::Translate('editor.allEditors'),
			FILTER_EDITOR_ME => Localex::Translate('editor.me')
		);

		$filterSectionOptions = array(
			FILTER_SECTION_ALL => Localex::Translate('editor.allSections')
		) + $sections;
	...
	function assignEditor($args) {
		$this->validate();
		Localex::requireComponents(array(LOCALE_COMPONENT_PKP_MANAGER)); // manager.people.noneEnrolled
