		...
		switch ($this->type) {
			...
			case PAYMENT_TYPE_SUBMISSION:
				if ( $journal->getLocalizedSetting('submissionFeeName') != '' ) {
					...
					return Localex::translate('payment.type.submission');
				}
	...
	function getAssocDescription() {
		...
		switch ($this->type) {
			...
			case PAYMENT_TYPE_RENEW_SUBSCRIPTION:
				...
				if ( !$subscription) return Localex::translate('manager.payment.notFound');
			...
			case PAYMENT_TYPE_PURCHASE_ARTICLE:
				...
				if ( !$article ) return Localex::translate('manager.payment.notFound');
