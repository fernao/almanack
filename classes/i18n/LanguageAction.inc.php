...
class LanguageAction {
	...
	function getDownloadableLocales() {
		...
		return Localex::loadLocaleList($descriptorFilename);
	}
	...
	function downloadLocale($locale, &$errors) {
		...
		if (!is_array($downloadableLocales) || !isset($downloadableLocales[$locale])) {
			$errors[] = Localex::translate('admin.languages.download.cannotOpen');
			return false;
		}
		...
		if (!isset($locales[$locale])) {
			...
			if ($pos === false) {
				...
				$errors[] = Localex::translate('admin.languages.download.cannotModifyRegistry');
				return false;
			}
