...
class Localex extends PKPLocale {
	...
	function getSupportedLocales() {
		...
		if (!isset($supportedLocales)) {
			if (defined('SESSION_DISABLE_INIT') || !Config::getVar('general', 'installed')) {
				$supportedLocales = Localex::getAllLocales();
	...
	function getSupportedFormLocales() {
		...
		if (!isset($supportedFormLocales)) {
			if (defined('SESSION_DISABLE_INIT') || !Config::getVar('general', 'installed')) {
				$supportedFormLocales = Localex::getAllLocales();
			} elseif (($journal =& Request::getJournal())) {
	...
	function getLocale() {
		...
		if (!isset($currentLocale)) {
			if (defined('SESSION_DISABLE_INIT') || !Config::getVar('general', 'installed')) {
				...
				if (empty($locale) || !in_array($locale, array_keys(Localex::getSupportedLocales()))) $locale = Request::getCookieVar('currentLocale');
			} else {
	...
	function getLocalePrecedence() {
		...
		if (!isset($localePrecedence)) {
			$localePrecedence = array(Localex::getLocale());
	...
	function getPrimaryLocale() {
		...
		if (!isset($locale) || !Localex::isLocaleValid($locale)) {
			$locale = LOCALE_DEFAULT;
		}
