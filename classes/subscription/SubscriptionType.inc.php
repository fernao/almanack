...
class SubscriptionType extends DataObject {
	...
	function getDurationYearsMonths() {
		if ($this->getData('nonExpiring')) {
			return Localex::translate('subscriptionTypes.nonExpiring');
		}
		...
		if ($years == 1) {
			$yearsMonths = '1 ' . Localex::translate('subscriptionTypes.year');
		} elseif ($years > 1) {
			$yearsMonths = $years . ' ' . Localex::translate('subscriptionTypes.years');
		}

		if ($months == 1) {
			$yearsMonths .= $yearsMonths == ''  ? '1 ' : ' 1 ';
			$yearsMonths .= Localex::translate('subscriptionTypes.month'); 
		} elseif ($months > 1){
			$yearsMonths .= $yearsMonths == ''  ? $months . ' ' : ' ' . $months . ' ';
			$yearsMonths .= Localex::translate('subscriptionTypes.months');
		}
