...
class InstallForm extends Form {
	...
	function InstallForm() {
		...
		$this->supportedLocales = Localex::getAllLocales();
		$this->localesComplete = array();
		foreach ($this->supportedLocales as $key => $name) {
			$this->localesComplete[$key] = Localex::isLocaleComplete($key);
		}
		...
		$this->supportedConnectionCharsets = array (
			'' => Localex::translate('common.notApplicable'),
			'utf8' => 'Unicode (UTF-8)'
		);
	...
	function display() {
		...
		$templateMgr->assign('supportsMBString', String::hasMBString() ? Localex::translate('common.yes') : Localex::translate('common.no'));
	...
	function dbInstallError($errorMsg) {
		$templateMgr =& TemplateManager::getManager();
		if (empty($errorMsg)) $errorMsg = Localex::translate('common.error.databaseErrorUnknown');
