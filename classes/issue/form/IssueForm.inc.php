...
class IssueForm extends Form {
	...
	function display() {
		...
		$templateMgr->assign('accessOptions', array(
			ISSUE_ACCESS_OPEN => Localex::Translate('editor.issues.openAccess'),
			ISSUE_ACCESS_SUBSCRIPTION => Localex::Translate('editor.issues.subscription')
		));
	...
	function validate($issueId = 0) {
		...
		if ($publicIssueId && $issueDao->publicIssueIdExists($publicIssueId, $issueId, $journal->getId())) {
			$this->addError('publicIssueId', Localex::translate('editor.issues.issuePublicIdentificationExists'));
		...
		if ($publicFileManager->uploadedFileExists('coverPage')) {
			...
			if (!$publicFileManager->getImageExtension($type)) {
				$this->addError('coverPage', Localex::translate('editor.issues.invalidCoverPageFormat'));
		...
		if ($publicFileManager->uploadedFileExists('styleFile')) {
			...
			if ($type != 'text/plain' && $type != 'text/css') {
				$this->addError('styleFile', Localex::translate('editor.issues.invalidStyleFormat'));
			}
		}
