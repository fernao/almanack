...
class JournalSettingsDAO extends DAO {
		...
		foreach ($tree->getChildren() as $setting) {
			...
			if (isset($nameNode) && isset($valueNode)) {
				...
				$this->updateSetting(
					$journalId,
					$name,
					$isLocaleField?array(Localex::getLocale() => $value):$value,
					$type,
					$isLocaleField
				);
			}
	...
	function _performLocalizedReplacement($rawInput, $paramArray = array(), $locale = null) {
		...
		if ( isset($matches[1]) ) {
			Localex::requireComponents(array(LOCALE_COMPONENT_OJS_DEFAULT, LOCALE_COMPONENT_OJS_MANAGER), $locale);
			return Localex::translate($matches[1], $paramArray, $locale);
		}
	...
	function _installer_regexp_callback($matches) {
		return Localex::translate($matches[1]);
	}
}
