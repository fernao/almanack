...
class Journal extends DataObject {
	...
	function &getSupportedLocaleNames() {
		...
		if (!isset($supportedLocales)) {
			...
			$localeNames =& Localex::getAllLocales();
	...
	function getLocalizedPageHeaderTitle($home = false) {
		...
		foreach (array(Localex::getLocale(), Localex::getPrimaryLocale()) as $locale) {
	...
	function getLocalizedDescription() {
		return $this->getDescription(Localex::getLocale());
	}
	...
	function &getLocalizedSetting($name) {
		$returner = $this->getSetting($name, Localex::getLocale());
		if ($returner === null) {
			unset($returner);
			$returner = $this->getSetting($name, Localex::getPrimaryLocale());
		}
		return $returner;
	}
