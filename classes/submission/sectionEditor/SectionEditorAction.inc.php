...
class SectionEditorAction extends Action {
	...
	function recordDecision($sectionEditorSubmission, $decision) {
		...
		if (!HookRegistry::call('SectionEditorAction::recordDecision', array(&$sectionEditorSubmission, $editorDecision))) {
			...
			Localex::requireComponents(array(LOCALE_COMPONENT_APPLICATION_COMMON, LOCALE_COMPONENT_OJS_EDITOR));
			ArticleLog::logEvent($sectionEditorSubmission->getArticleId(), ARTICLE_LOG_EDITOR_DECISION, ARTICLE_LOG_TYPE_EDITOR, $user->getId(), 'log.editor.decision', array('editorName' => $user->getFullName(), 'articleId' => $sectionEditorSubmission->getArticleId(), 'decision' => Localex::translate($decisions[$decision])));
		}
	}
			...
			if (!$email->isEnabled() || ($send && !$email->hasErrors())) {
				...
				if (!Request::getUserVar('continued')) {
					...
					$paramArray = array(
						...
						'abstractTermIfEnabled' => ($sectionEditorSubmission->getLocalizedAbstract() == ''?'':Localex::translate('article.abstract')),
			...
			if (!Request::getUserVar('continued')) {
				...
				foreach ($reviewAssignments as $reviewAssignment) {
					...
					if ($reviewAssignment->getDateCompleted() != null && !$reviewAssignment->getCancelled()) {
						...
						if($articleComments) {
							$body .= "------------------------------------------------------\n";
							$body .= Localex::translate('submission.comments.importPeerReviews.reviewerLetter', array('reviewerLetter' => chr(ord('A') + $reviewIndexes[$reviewAssignment->getId()]))) . "\n";
