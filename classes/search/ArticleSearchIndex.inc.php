...
class ArticleSearchIndex {
	...
	function rebuildIndex($log = false) {
		...
		while (!$journals->eof()) {
			...
			// fix: checks if section must indexed or not
			$section = array();
			$sectionsResult =& $sectionDao->getJournalSections($journal->getJournalId());
			$sectionsResult =& $sectionsResult->toArray();
			foreach ($sectionsResult as $result) {
			  $sections[$result->getSectionId()] = $result;
			}
			...
			while (!$articles->eof()) {
				...
				if ($sections[$article->getSectionId()]->getMetaIndexed()) { 
