...
class ArticleReportPlugin extends ReportPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.reports.articles.displayName');
	}

	function getDescription() {
		return Localex::translate('plugins.reports.articles.description');
	}
	...
	function display(&$args) {
		...
		Localex::requireComponents(array(LOCALE_COMPONENT_OJS_EDITOR, LOCALE_COMPONENT_PKP_SUBMISSION));

		import('classes.article.Article');
		$decisionMessages = array(
			SUBMISSION_EDITOR_DECISION_ACCEPT => Localex::translate('editor.article.decision.accept'),
			SUBMISSION_EDITOR_DECISION_PENDING_REVISIONS => Localex::translate('editor.article.decision.pendingRevisions'),
			SUBMISSION_EDITOR_DECISION_RESUBMIT => Localex::translate('editor.article.decision.resubmit'),
			SUBMISSION_EDITOR_DECISION_DECLINE => Localex::translate('editor.article.decision.decline'),
			null => Localex::translate('plugins.reports.articles.nodecision')
		);

		$columns = array(
			'article_id' => Localex::translate('article.submissionId'),
			'title' => Localex::translate('article.title'),
			'abstract' => Localex::translate('article.abstract')
		);
			
		for ($a = 1; $a <= $maxAuthors; $a++) {
			$columns = array_merge($columns, array(
				'fname' . $a => Localex::translate('user.firstName') . " (" . Localex::translate('user.role.author') . " $a)",
				'mname' . $a => Localex::translate('user.middleName') . " (" . Localex::translate('user.role.author') . " $a)",
				'lname' . $a => Localex::translate('user.lastName') . " (" . Localex::translate('user.role.author') . " $a)",
				'country' . $a => Localex::translate('common.country') . " (" . Localex::translate('user.role.author') . " $a)",
				'affiliation' . $a => Localex::translate('user.affiliation') . " (" . Localex::translate('user.role.author') . " $a)",
				'email' . $a => Localex::translate('user.email') . " (" . Localex::translate('user.role.author') . " $a)",
				'url' . $a => Localex::translate('user.url') . " (" . Localex::translate('user.role.author') . " $a)",
				'biography' . $a => Localex::translate('user.biography') . " (" . Localex::translate('user.role.author') . " $a)"
			));
		}
			
		$columns = array_merge($columns, array(
			'section_title' => Localex::translate('section.title'),
			'language' => Localex::translate('common.language'),
			'editor_decision' => Localex::translate('submission.editorDecision'),
			'status' => Localex::translate('common.status')
		));
		...
		while ($row =& $articlesIterator->next()) {
			...
			foreach ($columns as $index => $junk) {
				if ($index == 'editor_decision') {
					...
					$columns[$index] = Localex::translate($statusMap[$row[$index]]);
