...
class CounterHandler extends Handler {
	...
	*/
	function _assignTemplateCounterXML($templateManager, $begin, $end='') {
		...
		$siteTitle = $siteSettingsDao->getSetting('title',Localex::getLocale());
	...
	*/
	function report() {
		...
		String::fputcsv($fp, array(Localex::translate('plugins.generic.counter.1a.title1')));
		String::fputcsv($fp, array(Localex::translate('plugins.generic.counter.1a.title2', array('year' => $year))));
		String::fputcsv($fp, array()); // FIXME: Criteria should be here?
		String::fputcsv($fp, array(Localex::translate('plugins.generic.counter.1a.dateRun')));
		String::fputcsv($fp, array(strftime("%Y-%m-%d")));

		$cols = array(
			'',
			Localex::translate('plugins.generic.counter.1a.publisher'),
			Localex::translate('plugins.generic.counter.1a.platform'),
			Localex::translate('plugins.generic.counter.1a.printIssn'),
			Localex::translate('plugins.generic.counter.1a.onlineIssn')
		);
		for ($i=1; $i<=12; $i++) {
		...
		$cols[] = Localex::translate('plugins.generic.counter.1a.ytdTotal');
		$cols[] = Localex::translate('plugins.generic.counter.1a.ytdHtml');
		$cols[] = Localex::translate('plugins.generic.counter.1a.ytdPdf');
		fputcsv($fp, $cols);

		// Display the totals first
		$totals = $counterReportDao->getMonthlyTotalRange($begin, $end);
		$cols = array(
			Localex::translate('plugins.generic.counter.1a.totalForAllJournals'),
			'-', // Publisher
			'', // Platform
			'-',
			'-'
