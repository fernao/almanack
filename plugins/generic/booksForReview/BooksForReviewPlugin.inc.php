...
class BooksForReviewPlugin extends GenericPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.generic.booksForReview.displayName');
	}

	function getDescription() {
		return Localex::translate('plugins.generic.booksForReview.description');
	}
		...
		if ($page == 'editor') {
			...
			if ($op) {
				...
				if (in_array($op, $editorPages)) {
					...
					Localex::requireComponents(array(LOCALE_COMPONENT_APPLICATION_COMMON, LOCALE_COMPONENT_PKP_USER, LOCALE_COMPONENT_OJS_EDITOR));
		...
		if ($page == 'author') {
			...
			if ($op) {
				...
				if (in_array($op, $authorPages)) {
					...
					define('BOOKS_FOR_REVIEW_PLUGIN_NAME', $this->getName());
					Localex::requireComponents(array(LOCALE_COMPONENT_APPLICATION_COMMON, LOCALE_COMPONENT_PKP_USER, LOCALE_COMPONENT_OJS_AUTHOR));
					$handlerFile =& $params[2];
		...
		if ($page == 'booksForReview') {
			...
			if ($op) {
				...
				if (in_array($op, $publicPages)) {
					...
					define('BOOKS_FOR_REVIEW_PLUGIN_NAME', $this->getName());
					Localex::requireComponents(array(LOCALE_COMPONENT_APPLICATION_COMMON));
					$handlerFile =& $params[2];
	...
	function displayArticleCoverPageAbstract($hookName, $params) {
		if ($this->getEnabled()) {
			...
			if ($book) {
				...
				$smarty->assign('locale', Localex::getLocale());
				$smarty->assign('book', $book);
