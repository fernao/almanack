...
class BookForReviewForm extends Form {
	...
	function BookForReviewForm($parentPluginName, $bookId = null) {
		...
		$this->validStatus = array (
			BFR_STATUS_AVAILABLE => Localex::translate('plugins.generic.booksForReview.status.available'),
			BFR_STATUS_REQUESTED => Localex::translate('plugins.generic.booksForReview.status.requested'),
			BFR_STATUS_ASSIGNED => Localex::translate('plugins.generic.booksForReview.status.assigned'),
			BFR_STATUS_MAILED => Localex::translate('plugins.generic.booksForReview.status.mailed'),
			BFR_STATUS_SUBMITTED => Localex::translate('plugins.generic.booksForReview.status.submitted')
		);

		$this->validAuthorTypes = array (
			BFR_AUTHOR_TYPE_BY => Localex::translate('plugins.generic.booksForReview.authorType.by'),
			BFR_AUTHOR_TYPE_EDITED_BY => Localex::translate('plugins.generic.booksForReview.authorType.editedBy')
		);
	...
	function validate() {
		...
		if ($publicFileManager->uploadedFileExists(BFR_COVER_PAGE_IMAGE_NAME)) {
			...
			if (!$extension) {
				// Not a valid image.
				$this->addError('imageFile', Localex::translate('submission.layout.imageInvalid'));
				return false;
			}
		}
