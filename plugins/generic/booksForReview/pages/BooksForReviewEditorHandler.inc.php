				...
				// Display mail form for editor
					...
					if (!$request->getUserVar('continued')) {
						...
						if (empty($userMailingAddress)) {
							$userMailingAddress = Localex::translate('plugins.generic.booksForReview.editor.noMailingAddress'); 
						} else {
							$countryDao =& DAORegistry::getDAO('CountryDAO');
							$countries =& $countryDao->getCountries();
	...
	function setupTemplate($subclass = false) {
		...
		if ($subclass) {
			...
			$pageCrumbs[] = array(
				...
				Localex::Translate('plugins.generic.booksForReview.displayName'),
				true
			);
		}
