...
class AuthorDepositForm extends Form {
	...
	function execute() {
		...
		if ($allowAuthorSpecify && $authorDepositUrl != '') {
			...
			$notificationManager->createTrivialNotification(Localex::translate('notification.notification'), Localex::translate('plugins.generic.sword.depositComplete', array('itemTitle' => $this->article->getLocalizedTitle(), 'repositoryName' => $this->getData('authorDepositUrl'))), NOTIFICATION_TYPE_SUCCESS, null, false);
		}
		...
		foreach ($depositableDepositPoints as $key => $depositPoint) {
			...
			$notificationManager->createTrivialNotification(Localex::translate('notification.notification'), Localex::translate('plugins.generic.sword.depositComplete', array('itemTitle' => $this->article->getLocalizedTitle(), 'repositoryName' => $depositPoint['name'])), NOTIFICATION_TYPE_SUCCESS, null, false);
		}
