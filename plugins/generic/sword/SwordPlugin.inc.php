...
class SwordPlugin extends GenericPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.generic.sword.displayName');
	}
	...
	function getDescription() {
		return Localex::translate('plugins.generic.sword.description');
	}
	...
	function callbackAuthorDeposits($hookName, $args) {
		...
		foreach ($depositPoints as $depositPoint) {
			...
			$notificationManager->createTrivialNotification(Localex::translate('notification.notification'), Localex::translate('plugins.generic.sword.automaticDepositComplete', array('itemTitle' => $sectionEditorSubmission->getLocalizedTitle(), 'repositoryName' => $depositPoint['name'])), NOTIFICATION_TYPE_SUCCESS, null, false);
		}
	...
	function getManagementVerbs() {
		...
		if ($this->getEnabled()) {
			...
			$verbs[] = array(
				'settings',
				Localex::translate('plugins.generic.sword.settings')
			);
		} else {
			$verbs[] = array(
				'enable',
	...
	function manage($verb, $args, &$message) {
		...
		switch ($verb) {
			case 'settings':
				Localex::requireComponents(array(LOCALE_COMPONENT_APPLICATION_COMMON,  LOCALE_COMPONENT_PKP_MANAGER));
			...
			case 'enable':
				...
				$message = Localex::translate('plugins.generic.sword.enabled');
				$returner = false;
				break;
			case 'disable':
				$this->updateSetting($journal->getId(), 'enabled', false);
				$message = Localex::translate('plugins.generic.sword.disabled');
				$returner = false;
				break;
