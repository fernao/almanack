...
class CustomLocalePlugin extends GenericPlugin {
			...
			if ($this->getEnabled()) {
				...
				foreach ($localeFiles as $localeFile) {
					...
					if (FileManager::fileExists($customLocalePath)) {
						Localex::registerLocaleFile($locale, $customLocalePath, true);
					}
				}

				// Add custom locale data for all locale files registered after this plugin
				HookRegistry::register('PKPLocalex::registerLocaleFile', array(&$this, 'addCustomLocale'));
			}
	...
	function getDisplayName() {
		return Localex::translate('plugins.generic.customLocale.name');
	}

	function getDescription() {
		return Localex::translate('plugins.generic.customLocale.description');
	}
	...
	function getManagementVerbs() {
		$verbs = array();
		if ($this->getEnabled()) {
			$verbs[] = array('index', Localex::translate('plugins.generic.customLocale.customize'));
		}
