...
class OpenAdsPlugin extends GenericPlugin {
	...
	function getDisplayName() {
		$this->addLocaleData();
		return Localex::translate('plugins.generic.openads');
	}

	function getDescription() {
		$this->addLocaleData();
		return Localex::translate($this->isConfigured()?'plugins.generic.openads.description':'plugins.generic.openads.descriptionUnconfigured');
	}
	...
	function rtOutputFilter($output, &$smarty) {
		...
		$index = strrpos($output, '<h5>' . Localex::translate('rt.readingTools') . '</h5>');
	...
	function getManagementVerbs() {
		$verbs = array();
		if ($this->getEnabled()) {
			$verbs[] = array('disable', Localex::translate('common.disable'));
			if ($this->isConfigured()) {
				$verbs[] = array('settings', Localex::translate('plugins.generic.openads.manager.settings'));
			}
		} else {
			if ($this->isConfigured()) {
				$verbs[] = array('enable', Localex::translate('common.enable'));
			}
		}
		return $verbs;
	}
