...
class OpenAdsConnection {
	...
	function loadConfig() {
		...
		if (!$config) {
			$this->errors[] = Localex::translate('plugins.generic.openads.error.configFileNotFound', array('filename' => $configFilename));
			return false;
		}
		...
		if (	!isset($config['database']['type']) ||
			...
			$this->errors[] = Localex::translate('plugins.generic.openads.error.missingParameter');
			return false;
		}
	...
	function &getConnection() {
		...
		if (!$conn) {
			$this->errors[] = Localex::translate('plugins.generic.openads.error.dbConnectionError');
			$returner = false;
			return $returner;
		}
