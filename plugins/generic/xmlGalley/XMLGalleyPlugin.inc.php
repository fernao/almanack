...
class XMLGalleyPlugin extends GenericPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.generic.xmlGalley.displayName');
	}

	function getDescription() {
		return Localex::translate('plugins.generic.xmlGalley.description');
	}
	...
	function getManagementVerbs() {
		$verbs = array();
		if ($this->getEnabled()) {
			$verbs[] = array('settings', Localex::translate('plugins.generic.xmlGalley.manager.settings'));
		}
			...
			case 'test':
				...
				if ($xsltRenderer == "external") {
					...
					if (trim(preg_replace("/\s+/", " ", $result)) != "Open Journal Systems Success" ) {
						$form->addError('content', Localex::translate('plugins.generic.xmlGalley.settings.externalXSLTFailure'));
			...
			case 'settings':
				...
				// if we are uploading a custom XSL sheet
					...
					if ($fileManager->uploadedFileExists('customXSL')) {
						...
						} else $form->addError('content', Localex::translate('plugins.generic.xmlGalley.settings.customXSLInvalid'));

					} else $form->addError('content', Localex::translate('plugins.generic.xmlGalley.settings.customXSLRequired'));
