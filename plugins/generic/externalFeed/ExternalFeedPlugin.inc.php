...
class ExternalFeedPlugin extends GenericPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.generic.externalFeed.displayName');
	}

	function getDescription() {
		return Localex::translate('plugins.generic.externalFeed.description');
	}
	...
	function getManagementVerbs() {
		$verbs = array();
		if ($this->getEnabled()) {
			$verbs[] = array('feeds', Localex::translate('plugins.generic.externalFeed.manager.feeds'));
			$verbs[] = array('settings', Localex::translate('plugins.generic.externalFeed.manager.settings'));
		}
			...
			if (empty($requestedPage) || $requestedPage == 'index') {
				...
				while ($currentFeed =& $feeds->next()) {
					...
					foreach ($feed->get_items(0, $recentItems) as $item):
						...
						$output .= Localex::Translate('plugins.generic.externalFeed.posted') . ': ' . date('Y-m-d', strtotime($item->get_date()));
						$output .= '</td>';
						$output .= '<td class="more">';
						$output .= '<a href="' . $item->get_permalink() . '" target="_blank">' . Localex::Translate('plugins.generic.externalFeed.more') . '</a>';
						$output .= '</td>';
						$output .= '</tr>';
