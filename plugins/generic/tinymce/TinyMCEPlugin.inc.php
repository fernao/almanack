...
class TinyMCEPlugin extends GenericPlugin {
	...
	function callback($hookName, $args) {
		...
		if (!empty($enableFields)) {
			...
			$allLocales = Localex::getAllLocales();
			$localeList = array();
			...
			<script language="javascript" type="text/javascript">
				tinyMCE.init({
					...
					plugins : "paste,ibrowser,fullscreen",
					mode : "exact",
					language : "' . String::substr(Localex::getLocale(), 0, 2) . '",
					...
					theme_advanced_buttons3 : "",
				        theme_advanced_text_colors : "FFFFF,EEEEE,DDDDDD,CCCCCC,AAAAAA,FF0000,FFFF00,00FF00,0000FF"
				});
			</script>';
	...
	function getDisplayName() {
		return Localex::translate('plugins.generic.tinymce.name');
	}
	...
	function getDescription() {
		if ($this->isMCEInstalled()) return Localex::translate('plugins.generic.tinymce.description');
		return Localex::translate('plugins.generic.tinymce.descriptionDisabled', array('tinyMcePath' => TINYMCE_INSTALL_PATH));
	}
