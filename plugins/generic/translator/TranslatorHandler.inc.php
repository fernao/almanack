...
class TranslatorHandler extends Handler {
	...
	function index() {
		...
		$templateMgr->assign('locales', new ArrayItemIterator(Localex::getAllLocales(), $rangeInfo->getPage(), $rangeInfo->getCount()));
	...
	function setupTemplate($subclass = true) {
		...
		$templateMgr =& TemplateManager::getManager();
		Localex::requireComponents(array(LOCALE_COMPONENT_PKP_ADMIN, LOCALE_COMPONENT_PKP_MANAGER));
	...
	function edit($args) {
		...
		if (!Localex::isLocaleValid($locale)) Request::redirect(null, null, 'index');
