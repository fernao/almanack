...
class TranslatorAction {
	...
	function getLocaleFiles($locale) {
		if (!Localex::isLocaleValid($locale)) return null;

		$localeFiles = Localex::getFilenameComponentMap($locale);
	...
	function testLocale($locale, $referenceLocale) {
		$localeFileNames = Localex::getFilenameComponentMap($locale);
	...
	function testEmails($locale, $referenceLocale) {
		...
		foreach ($emails as $emailKey => $email) {
			...
			$bodyParams = Localex::getParameterNames($email['body']);
			$referenceBodyParams = Localex::getParameterNames($referenceEmails[$emailKey]['body']);
			...
			$subjectParams = Localex::getParameterNames($email['subject']);
			$referenceSubjectParams = Localex::getParameterNames($referenceEmails[$emailKey]['subject']);
