	...
	function execute() {
		...
		if (empty($uploadCode) || ($uploadCode != $submittedUploadCode)) {
			...
			if (!empty($thesisPhone)) {
				$thesisContactSignature .= "\n" . Localex::Translate('user.phone') . ': ' . $thesisPhone;
			}
			if (!empty($thesisFax)) {
				$thesisContactSignature .= "\n" . Localex::Translate('user.fax') . ': ' . $thesisFax;
			}

			$thesisContactSignature .= "\n" . Localex::Translate('user.email') . ': ' . $thesisEmail;
			...
			$paramArray = array(
				...
				'degree' => Localex::Translate($thesis->getDegreeString()),
