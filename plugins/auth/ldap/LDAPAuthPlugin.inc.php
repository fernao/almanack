...
class LDAPAuthPlugin extends AuthPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.auth.ldap.displayName');
	}
	...
	function getDescription() {
		return Localex::translate('plugins.auth.ldap.description');
	}
	...
	function userFromAttr(&$user, &$uattr) {
		...
		if (isset($affiliation))
			$user->setAffiliation($affiliation, Localex::getLocale());
		if (isset($email))
			$user->setEmail($email);
		if (isset($phone))
			$user->setPhone($phone);
		...
		if (isset($biography))
			$user->setBiography($biography, Localex::getLocale());
		if (isset($interests))
			$user->setInterests($interests, Localex::getLocale());
	}
	...
	function userToAttr(&$user, &$attr) {
		...
		if ($user->getAffiliation())
			$attr['organizationName'] = $user->getAffiliation(Localex::getLocale());
		if ($user->getEmail())
