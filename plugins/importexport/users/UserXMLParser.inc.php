...
class UserXMLParser {
	...
	function &parseData($file) {	
		...
		$journalPrimaryLocale = Localex::getPrimaryLocale();
						...
						switch ($attrib->getName()) {
							...
							case 'password':
								...
								if (isset($encrypted) && $encrypted !== 'plaintext') {
									...
									if ($encrypted != $ojsEncryptionScheme) {
										$this->errors[] = Localex::translate('plugins.importexport.users.import.encryptionMismatch', array('importHash' => $encrypted, 'ojsHash' => $ojsEncryptionScheme));
									}
							...
							case 'locales':
								...
								foreach (explode(':', $attrib->getValue()) as $locale) {
									if (Localex::isLocaleValid($locale) && in_array($locale, $siteSupportedLocales)) {
			...
			if ($newUsername || !$userExists) {
				...
				if (!$userDao->insertUser($user)) {
					...
					$this->errors[] = sprintf('%s: %s (%s)',
						Localex::translate('manager.people.importUsers.failedToImportUser'),
				...
				if (!$roleDao->roleExists($role->getJournalId(), $role->getUserId(), $role->getRoleId())) {
					if (!$roleDao->insertRole($role)) {
						...
						$this->errors[] = sprintf('%s: %s - %s (%s)',
							Localex::translate('manager.people.importUsers.failedToImportRole'),
							$role->getRoleName(),
							$user->getFullName(), $user->getUsername());
