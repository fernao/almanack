...
class CrossRefExportPlugin extends ImportExportPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.importexport.crossref.displayName');
	}

	function getDescription() {
		return Localex::translate('plugins.importexport.crossref.description');
	}
...
//		$command = array_shift($args);
		...
		if (!$journal) {
			if ($journalPath != '') {
				echo Localex::translate('plugins.importexport.crossref.cliError') . "\n";
				echo Localex::translate('plugins.importexport.crossref.error.unknownJournal', array('journalPath' => $journalPath)) . "\n\n";
			}
		...
		if ($xmlFile != '') switch (array_shift($args)) {
			case 'articles':
				...
				if (!$this->exportArticles($journal, $results, $xmlFile)) {
					echo Localex::translate('plugins.importexport.crossref.cliError') . "\n";
					echo Localex::translate('plugins.importexport.crossref.export.error.couldNotWrite', array('fileName' => $xmlFile)) . "\n\n";
				}
				return;
			case 'issue':
				$issueId = array_shift($args);
				...
				if ($issue == null) {
					echo Localex::translate('plugins.importexport.crossref.cliError') . "\n";
					echo Localex::translate('plugins.importexport.crossref.export.error.issueNotFound', array('issueId' => $issueId)) . "\n\n";
					return;
				}
				$issues = array($issue);
	...
	function usage($scriptName) {
		echo Localex::translate('plugins.importexport.crossref.cliUsage', array(
