...
class NativeImportExportPlugin extends ImportExportPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.importexport.native.displayName');
	}

	function getDescription() {
		return Localex::translate('plugins.importexport.native.description');
	}
	...
	function display(&$args) {
		...
		switch (array_shift($args)) {
			...
			case 'issues':
				...
				Localex::requireComponents(array(LOCALE_COMPONENT_OJS_EDITOR));
				$issueDao =& DAORegistry::getDAO('IssueDAO');
			...
			case 'import':
				...
				if (substr($this->getRootNodeName($doc), 0, 7) === 'article') {
					...
					if (!isset($context['issue']) || !isset($context['section'])) {
						$issues =& $issueDao->getIssues($journal->getId(), Handler::getRangeInfo('issues'));
						$templateMgr->assign_by_ref('issues', $issues);
						$templateMgr->assign('sectionOptions', array('0' => Localex::translate('author.submit.selectSection')) + $sectionDao->getSectionTitles($journal->getId(), false));
	...
	function executeCLI($scriptName, &$args) {
		...
		if (!$journal) {
			if ($journalPath != '') {
				echo Localex::translate('plugins.importexport.native.cliError') . "\n";
				echo Localex::translate('plugins.importexport.native.error.unknownJournal', array('journalPath' => $journalPath)) . "\n\n";
			}
		...
		switch ($command) {
			case 'import':
				...
				if (!$user) {
					if ($userName != '') {
						echo Localex::translate('plugins.importexport.native.cliError') . "\n";
						echo Localex::translate('plugins.importexport.native.error.unknownUser', array('userName' => $userName)) . "\n\n";
					}
					$this->usage($scriptName);
					return;
				}
				...
				switch ($this->getRootNodeName($doc)) {
					...
					case 'articles':
						...
						if (!$issue) {
							echo Localex::translate('plugins.importexport.native.cliError') . "\n";
							echo Localex::translate('plugins.importexport.native.export.error.issueNotFound', array('issueId' => $issueId)) . "\n\n";
							return;
						}
						...
						if (!$section) {
							echo Localex::translate('plugins.importexport.native.cliError') . "\n";
							echo Localex::translate('plugins.importexport.native.export.error.sectionNotFound', array('sectionIdentifier' => $sectionIdentifier)) . "\n\n";
							return;
						}
				...
				if ($result) {
					echo Localex::translate('plugins.importexport.native.import.success.description') . "\n\n";
					if (!empty($issues)) echo Localex::translate('issue.issues') . ":\n";
					foreach ($issues as $issue) {
						echo "\t" . $issue->getIssueIdentification() . "\n";
					}

					if (!empty($articles)) echo Localex::translate('article.articles') . ":\n";
					...
					foreach ($errors as $error) {
						echo "\t" . Localex::translate($error[0], $error[1]) . "\n";
					}
				}
				return;
				break;
				...
				if ($xmlFile != '') switch (array_shift($args)) {
					case 'article':
						...
						if ($publishedArticle == null) {
							echo Localex::translate('plugins.importexport.native.cliError') . "\n";
							echo Localex::translate('plugins.importexport.native.export.error.articleNotFound', array('articleId' => $articleId)) . "\n\n";
							return;
						}
						...
						if (!$this->exportArticle($journal, $issue, $section, $publishedArticle, $xmlFile)) {
							echo Localex::translate('plugins.importexport.native.cliError') . "\n";
							echo Localex::translate('plugins.importexport.native.export.error.couldNotWrite', array('fileName' => $xmlFile)) . "\n\n";
						}
						return;
					case 'articles':
	...
	function usage($scriptName) {
		echo Localex::translate('plugins.importexport.native.cliUsage', array(
