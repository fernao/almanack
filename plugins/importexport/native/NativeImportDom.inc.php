...
class NativeImportDom {
	...
	function importIssue(&$journal, &$issueNode, &$issue, &$errors, &$user, $isCommandLine, &$dependentItems, $cleanupErrors = true) {
		...
		if (($issueNode->getAttribute('identification') == 'title' || $issueNode->getAttribute('identification') == '') && (!$titleExists)) {
			...
			$issue->setTitle(Localex::translate('plugins.importexport.native.import.error.defaultTitle'), $journalPrimaryLocale);
			$hasErrors = true;
		}
	...
	function handleSuppFileNode(&$journal, &$suppNode, &$issue, &$section, &$article, &$errors, $isCommandLine, &$articleFileManager) {
		...
		switch (($suppType = $suppNode->getAttribute('type'))) {
			case 'research_instrument': $suppFile->setType(Localex::translate('author.submit.suppFile.researchInstrument')); break;
			case 'research_materials': $suppFile->setType(Localex::translate('author.submit.suppFile.researchMaterials')); break;
			case 'research_results': $suppFile->setType(Localex::translate('author.submit.suppFile.researchResults')); break;
			case 'transcripts': $suppFile->setType(Localex::translate('author.submit.suppFile.transcripts')); break;
			case 'data_analysis': $suppFile->setType(Localex::translate('author.submit.suppFile.dataAnalysis')); break;
			case 'data_set': $suppFile->setType(Localex::translate('author.submit.suppFile.dataSet')); break;
			case 'source_text': $suppFile->setType(Localex::translate('author.submit.suppFile.sourceText')); break;
