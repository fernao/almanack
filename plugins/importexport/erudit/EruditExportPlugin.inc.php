...
class EruditExportPlugin extends ImportExportPlugin {
	...
	function getDisplayName() {
		return Localex::translate('plugins.importexport.erudit.displayName');
	}

	function getDescription() {
		return Localex::translate('plugins.importexport.erudit.description');
	}
	...
	function executeCLI($scriptName, &$args) {
		...
		if (!$journal) {
			if ($journalPath != '') {
				echo Localex::translate('plugins.importexport.erudit.cliError') . "\n";
				echo Localex::translate('plugins.importexport.erudit.error.unknownJournal', array('journalPath' => $journalPath)) . "\n\n";
			}
		...
		if ($publishedArticle == null) {
			echo Localex::translate('plugins.importexport.erudit.cliError') . "\n";
			echo Localex::translate('plugins.importexport.erudit.export.error.articleNotFound', array('articleId' => $articleId)) . "\n\n";
			return;
		}
		...
		if (!isset($galley)) {
			echo Localex::translate('plugins.importexport.erudit.export.error.galleyNotFound', array('galleyLabel' => $galleyLabel)) . "\n\n";
			return;
		}
		$issue =& $issueDao->getIssueById($publishedArticle->getIssueId());
		if (!$this->exportArticle($journal, $issue, $publishedArticle, $galley, $xmlFile)) {
			echo Localex::translate('plugins.importexport.erudit.cliError') . "\n";
			echo Localex::translate('plugins.importexport.erudit.export.error.couldNotWrite', array('fileName' => $xmlFile)) . "\n\n";
		}
	}
	...
	function usage($scriptName) {
		echo Localex::translate('plugins.importexport.erudit.cliUsage', array(
