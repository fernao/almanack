...
class PKPLocale {
	...
	function translate($key, $params = array(), $locale = null) {
		if (!isset($locale)) $locale = Localex::getLocale();
		if (($key = trim($key)) == '') return '';

		$localeFiles =& Localex::getLocaleFiles($locale);
		$value = '';
	...
	function initialize() {
		...
		Localex::registerLocaleFile($locale, "lib/pkp/locale/$locale/common.xml");
	}
	...
	function getFilenameComponentMap($locale) {
		...
		if (!isset($filenameComponentMap[$locale])) {
			$filenameComponentMap[$locale] = Localex::makeComponentMap($locale);
		}
	...
	function requireComponents($components, $locale = null) {
		...
		if ($locale === null) $locale = Localex::getLocale();
		$filenameComponentMap = Localex::getFilenameComponentMap($locale);
		foreach ($components as $component) {
			...
			Localex::registerLocaleFile($locale, $filename);
	...
	function &registerLocaleFile ($locale, $filename, $addToTop = false) {
		...
		HookRegistry::call('PKPLocalex::registerLocaleFile', array(&$locale, &$filename, &$addToTop));
		return $localeFile;
	}

	function getLocaleStyleSheet($locale) {
		$contents =& Localex::_getAllLocalesCacheContent();
	...
	function &getAllLocales() {
		$rawContents =& Localex::_getAllLocalesCacheContent();
		$allLocales = array();
	...
	function installLocale($locale) {
		...
		HookRegistry::call('PKPLocalex::installLocale', array(&$locale));
	}
	...
	function reloadLocale($locale) {
		Localex::uninstallLocale($locale);
		Localex::installLocale($locale);
	}
	...
	function get3LetterFrom2LetterIsoLanguage($iso2Letter) {
		...
		$locales =& Localex::_getAllLocalesCacheContent();
	...
	function get3LetterIsoFromLocale($locale) {
		...
		return Localex::get3LetterFrom2LetterIsoLanguage($iso2Letter);
	}
	...
	function getLocaleFrom3LetterIso($iso3Letter) {
		...
		if (count($localeCandidates) > 1) {
			...
			// supported locale.
			$supportedLocales = Localex::getSupportedLocales();
	...
	function &_getAllLocalesCacheContent() {
		...
		if ($contents === false) {
			$allLocalesCache =& Localex::_getAllLocalesCache();
	...
	function _allLocalesCacheMiss(&$cache, $id) {
		...
		if ($allLocales === null) {
			...
			$allLocales = Localex::loadLocaleList(LOCALE_REGISTRY_FILE);
			asort($allLocales);
