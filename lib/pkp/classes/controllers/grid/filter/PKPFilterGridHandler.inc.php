...
class PKPFilterGridHandler extends GridHandler {
	...
	function initialize(&$request) {
		...
		Localex::requireComponents(array(LOCALE_COMPONENT_PKP_MANAGER, LOCALE_COMPONENT_PKP_SUBMISSION));
	...
	function deleteFilter(&$args, &$request) {
		...
		if ($result) {
			...
			$json = new JSON('false', Localex::translate('manager.setup.filter.grid.errorDeletingFilter'));
		}
		return $json->getString();
	}
