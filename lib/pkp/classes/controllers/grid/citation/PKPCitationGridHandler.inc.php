...
class PKPCitationGridHandler extends GridHandler {
		...
		if ($citations->eof()) {
			...
			while (!$citations->eof()) {
				...
				if ($citation->getCitationState() < CITATION_APPROVED) {
					...
					// export then.
					$errorMessage = Localex::translate('submission.citations.editor.export.foundUnapprovedCitationsMessage');
					break;
				}
				unset($citation);
			}
			...
			if (is_null($errorMessage)) {
				...
				if (is_a($exportFilter, 'Filter')) {
					...
					if (empty($exportOutput)) {
						$errorMessage = Localex::translate('submission.citations.editor.export.noExportOutput', array('filterName' => $exportFilter->getDisplayName()));
					}
	...
	function deleteCitation(&$args, &$request) {
		...
		if ($result) {
			...
			$json = new JSON('false', Localex::translate('submission.citations.editor.citationlist.errorDeletingCitation'));
		}
		return $json->getString();
	}
	...
	function sendAuthorQuery(&$args, &$request) {
		...
		$json = new JSON('true',
			...
			.Localex::translate('submission.citations.editor.details.sendAuthorQuerySuccess')
			.'</span></div>');
		return $json->getString();
	}
