...
class InstallTool extends CommandLineTool {
	...
	function printTitle($title) {
		printf("\n%s\n%s\n%s\n", str_repeat('-', 80), Localex::translate($title), str_repeat('-', 80));
	}
	...
	function readParam($name, $prompt, $defaultValue = null) {
		do {
			if (isset($defaultValue)) {
				printf("%s (%s): ", Localex::translate($prompt), $defaultValue !== '' ? $defaultValue : Localex::translate('common.none'));
			} else {
				printf("%s: ", Localex::translate($prompt));
			}
	...
	function readParamBoolean($name, $prompt, $default = 'N') {
		if ($default == 'N') {
			printf("%s [y/N] ", Localex::translate($prompt));
			$value = $this->readInput();
	...
	function readParamOptions($name, $prompt, $options, $defaultValue = null, $allowMultiple = false) {
		do {
			printf("%s\n", Localex::translate($prompt));
			foreach ($options as $k => $v) {
				printf("  %-10s %s\n", '[' . $k . ']', $v);
			}
			if ($allowMultiple) {
				printf("  (%s)\n", Localex::translate('installer.form.separateMultiple'));
			}
			if (isset($defaultValue)) {
				printf("%s (%s): ", Localex::translate('common.select'), $defaultValue !== '' ? $defaultValue : Localex::translate('common.none'));
			} else {
				printf("%s: ", Localex::translate('common.select'));
			}
