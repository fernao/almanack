	 ...
	 */
	 function editCitations(&$request, &$submission) {
		...
		if ($citations->getCount() > 0) {
			$initialHelpMessage = Localex::translate('submission.citations.editor.details.pleaseClickOnCitationToStartEditing');
		} else {
			$articleMetadataUrl = $router->url($request, null, null, 'viewMetadata', $submission->getId());
			$initialHelpMessage = Localex::translate('submission.citations.editor.pleaseImportCitationsFirst', array('articleMetadataUrl' => $articleMetadataUrl));
		}
