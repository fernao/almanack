...
class Form {
	...
	function Form($template = null, $callHooks = true, $requiredLocale = null, $supportedLocales = null) {
		...
		if ($requiredLocale === null) $requiredLocale = Localex::getPrimaryLocale();
		$this->requiredLocale = $requiredLocale;
		if ($supportedLocales === null) $supportedLocales = Localex::getSupportedFormLocales();
	...
	function getDefaultFormLocale() {
		if (empty($formLocale)) $formLocale = Localex::getLocale();
	...
	function smartyFieldLabel($params, &$smarty) {
		...
		if (isset($params) && !empty($params)) {
			if (isset($params['key'])) {
				$params['label'] = Localex::translate($params['key'], $params);
			}
