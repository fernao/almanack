...
class PKPPlugin {
	...
	function register($category, $path) {
		...
		if ($this->getInstallEmailTemplateDataFile()) {
			...
			HookRegistry::register ('PKPLocalex::installLocale', array(&$this, 'installLocale'));
		}
	...
	function addLocaleData($locale = null) {
		if ($locale == '') $locale = Localex::getLocale();
		$localeFilename = $this->getLocaleFilename($locale);
		if ($localeFilename) {
			Localex::registerLocaleFile($locale, $this->getLocaleFilename($locale));
			return true;
		}
		return false;
	}
	...
	function installData($hookName, $args) {
		...
		if ($sql) {
			...
			Localex::requireComponents(array(LOCALE_COMPONENT_PKP_INSTALLER));
			$installer->setError(INSTALLER_ERROR_DB, str_replace('{$file}', $this->getInstallDataFile(), Localex::translate('installer.installParseDBFileError')));
			$result = false;
		}
		return false;
	}
	...
	function installEmailTemplates($hookName, $args) {
		...
		if ($sql === false) {
			...
			$installer->setError(INSTALLER_ERROR_DB, str_replace('{$file}', $this->getInstallDataFile(), Localex::translate('installer.installParseEmailTemplatesFileError')));
			$result = false;
		} else {
	...
	function updateSchema($hookName, $args) {
		...
		if ($sql) {
			...
			$installer->setError(INSTALLER_ERROR_DB, str_replace('{$file}', $this->getInstallSchemaFile(), Localex::translate('installer.installParseDBFileError')));
			$result = false;
		}
		return false;
	}
