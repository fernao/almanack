...
class PKPTemplateManager extends Smarty {
	...
	function PKPTemplateManager($request = null) {
		...
		if (($localeStyleSheet = Localex::getLocaleStyleSheet($locale)) != null) $this->addStyleSheet($request->getBaseUrl() . '/' . $localeStyleSheet);
	...
	function smartyTranslate($params, &$smarty) {
		if (isset($params) && !empty($params)) {
			if (!isset($params['key'])) return Localex::translate('');
			...
			return Localex::translate($key, $params);
		}
	}
		...
		if (isset($params['options'])) {
			if (isset($params['translateValues'])) {
				...
				foreach ($params['options'] as $k => $v) {
					$newOptions[Localex::translate($k)] = Localex::translate($v);
				}
		...
		if (isset($params) && !empty($params)) {
			...
			if (isset($params['name'])) {
				...
				if (isset($params['alt'])) {
					...
					$iconHtml .= Localex::translate('icon.'.$params['name'].'.alt');
				}
				$iconHtml .= '" ';
	...
	function smartyPageInfo($params, &$smarty) {
		...
		return Localex::translate('navigation.items', array(
			'from' => ($to===0?0:$from),
			'to' => $to,
			'total' => $itemTotal
		));
	...
	function smartySortHeading($params, &$smarty) {
		if (isset($params) && !empty($params)) {
			...
			$text = isset($params['key']) ? Localex::translate($params['key']) : '';
	...
	function smartyLoadUrlInDiv($params, &$smarty) {
		...
		if (isset($params['loadMessageId'])) {
			...
			$this->assign('inDivLoadMessage', Localex::translate($loadMessageId, $params));
		} else {
			$this->assign('inDivLoadMessage', Localex::translate('common.loading'));
		}
	...
	function smartyModal($params, &$smarty) {
		...
		$submitButton = Localex::translate('common.ok');
		$cancelButton = Localex::translate('common.cancel');
	...
	function smartyConfirm($params, &$smarty) {
		...
		if (isset($params['dialogText']))  {
			...
			if(isset($params['translate']) && $params['translate'] == false) {
				...
				$dialogText = Localex::translate($params['dialogText']);
			}
		} else {
			$showDialog = false;
		}
	...
	function smartyModalTitle($params, &$smarty) {
		...
		if(isset($params['key'])) {
			$keyHtml = "<span class='text'>" . Localex::translate($params['key']) . "</span>";
