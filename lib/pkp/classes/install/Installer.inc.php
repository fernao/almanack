...
class Installer {
	...
	function preInstall() {
		...
		if (!isset($this->locale)) {
			$this->locale = Localex::getLocale();
		}

		if (!isset($this->installedLocales)) {
			$this->installedLocales = array_keys(Localex::getAllLocales());
		}
	...
	function postInstall() {
		...
		if ($this->getParam('manualInstall')) {
			$this->log(Localex::translate('installer.pleaseUpgradeAfterManualInstall'));
		}
		...
		switch ($action['type']) {
			case 'schema':
				...
				if ($sql) {
					...
					$this->setError(INSTALLER_ERROR_DB, str_replace('{$file}', $fileName, Localex::translate('installer.installParseDBFileError')));
					return false;
				}
				break;
			case 'data':
	...
	function getErrorString() {
		switch ($this->getErrorType()) {
			...
			default:
				return Localex::translate($this->getErrorMsg());
		}
	}
