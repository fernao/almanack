		...
		if ($inputarr) {
			...
			$last = sizeof($sqlarr)-1;
                        ...
                        foreach($sqlarr as $k => $v) {
                               ...
                               // Added by asmecher:
                               // pg_query_params converts doubles to strings
                               // using localized standards, which may violate
                               // the SQL spec (i.e. using , instead of . for
                               // decimal point). Format numbers locally.
                               if (gettype($inputarr[$k]) == 'double') {
                                       $inputarr[$k] = str_replace(array($localedata['thousands_sep'], $localedata['decimal_point']), array('', '.'), $inputarr[$k]);
                               }
			}
