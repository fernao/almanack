...
class ModsSchemaSubmissionAdapter extends MetadataDataObjectAdapter {
	...
	function &injectMetadataIntoDataObject(&$modsDescription, &$submission, $replace, $authorClassName) {
		...
		$catalogingLocale = Localex::getLocaleFrom3LetterIso($catalogingLanguage);
		...
		$submissionLocale = Localex::get2LetterFrom3LetterIsoLanguage($submissionLanguage);
		if ($submissionLocale) {
	...
	function &extractMetadataFromDataObject(&$submission, $authorMarcrelatorRole = 'aut') {
		...
		$catalogingLocale = Localex::getPrimaryLocale();
		$catalogingLanguage = Localex::get3LetterIsoFromLocale($catalogingLocale);
		...
		// Submission language
		$submissionLanguage = Localex::get3LetterFrom2LetterIsoLanguage($submission->getLanguage());
